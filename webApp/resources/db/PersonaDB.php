...
class personaDB {
    ...
    public static function insertaPersona($persona) {
        ...
        $dbh = $conexion->getDbh();
        try {
            $consulta = 'INSERT INTO persona (nombre, a_paterno, a_materno, calle, numero, codigo_postal, ciudad, telefono, correo_electronico, creado) VALUES (?,?,?,?,?,?,?,?,?,now())';
            ...
            $stmt->bindParam(7, $persona['ciudad']);
            $stmt->bindParam(8, $persona['telefono']);
            $stmt->bindParam(9, $persona['email']);
            $stmt->execute();
